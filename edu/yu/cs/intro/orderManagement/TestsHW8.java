...
public class TestsHW8 {
	...
	public static void main(String[] args) {
		...
		// Resolved - but Judah's code made weird return for
	...
	public static void runDownStockViaOrdersRestoreToDefaultLevel() {
		...
		// System.out.println("We start this quant. of prod2: " +
		// warehouse.getStockLevel(2));
		warehouse.fulfill(2, 4);
		// System.out.println("After 4 of prod2 are ordered there are: " +
		// warehouse.getStockLevel(2));
		warehouse.fulfill(2, 1);
		// System.out.println("After 1 of prod2 are ordered there are: " +
		// warehouse.getStockLevel(2));
		...
		if (warehouse.getStockLevel(2) != 5) {
			...
			// throw new IncorrectBehaviorException("Stock level of product 2 was initially
			// 5, 4 were ordered and then 1 was ordered, warehouse.getStockLevel should now
			// return 5, but it returned " );
		} else {
	...
	public static void restockToLowerAndGetStockLevel() {
		...
		if (warehouse.getStockLevel(1) != 5) {
			System.out.println("Failed, Judah threw IncorrectBehaviorException");
			// throw new IncorrectBehaviorException("Stock level of product 1 was initially
			// 5, then set to 1 using warehouse.setDefaultStockLevel, but
			// warehouse.getStockLevel returned " + level);
		} else {
			System.out.println("Fixed");
		}
	...
	public static void successfulValidateProducts() {
		...
		OrderManagementSystem testOMS = new OrderManagementSystem(products, ogDefaultStock, svcPvdrs, wh);
		Order order = new Order();
		...
		if (ret != 0) {
			...
			// throw new IncorrectBehaviorException("validateProducts should've returned 0,
			// but returned " + ret);
		} else {
			System.out.println("Fixed");
		}
	...
	public static void unsuccessfulValidateServicesNotEnoughProviders() {
		...
		if (ret != 6 && ret != 2) {
			System.out.println("Failed: IncorrectBehaviorException thrown in Judah's code.");
			// throw new IncorrectBehaviorException("validateServices should've returned 6
			// or 2, but returned " + ret);
		} else {
			System.out.println("Fixed");
		}
	}
	...
	public static void testAddGetCatalog() {
		...
		// make sure that adding duplicates doesn't result in more products in the
		// catalog
	...
	public static void catalogMatchesExpectedSet(OrderManagementSystem system) {
		...
		if (prdcts.size() != catalog.size() || !catalog.containsAll(prdcts)) {
			System.out.println("Failed: IncorrectBehaviorException thrown in Judah's code.");
			// throw new IncorrectBehaviorException("Incorrect Set of Products returned from
			// OrderManagementSystem.getProductCatalog after products were added via
			// OrderManagementSystem.addNewProducts");
		} else {
			System.out.println("Fixed");
		}
	}
